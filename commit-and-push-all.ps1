...
# Führe dieses Skript in PowerShell aus: .\commit-and-push-all.ps1
...
# Prüfe ob CSV-Datei im Repository ist
Write-Host "🔍 Prüfe ob vokabeln.csv im Repository ist..." -ForegroundColor Yellow
$csvInRepo = git ls-files | Select-String "vokabeln.csv"
if ($csvInRepo) {
    Write-Host "✅ vokabeln.csv ist im Repository" -ForegroundColor Green
} else {
    Write-Host "⚠️ vokabeln.csv ist NICHT im Repository. Füge hinzu..." -ForegroundColor Yellow
    git add backend/vokabeln.csv
    Write-Host "✅ vokabeln.csv hinzugefügt" -ForegroundColor Green
}
Write-Host ""
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "4. Suche nach: '📚 Keine Vokabeln gefunden. Starte automatischen Import...'" -ForegroundColor White
} else {
